...
## Cell type circos plot
m3plot <- read.csv("Circos_Cell_type.csv", header=T)
MC <- t(m3plot[, 1])
Code <- t(m3plot[, 2])
rownames(Code) = "Module Color"
m3plot1 <- t(m3plot[, 3:9])
lable1=row.names(m3plot1);lable1
lable2=m3plot$Module;lable2
lable3=row.names(Code);lable3
colfunc<-colorRampPalette(c("yellow","red"))
cuts <- c(1.3, 2, 3, 4, 5, 6, 7, 8, 9, 10)
m3plot2 <- m3plot1
i <- 1
m3plot2[m3plot1 < cuts[i]] <- "#E6E6E6"
for(i in 1:9){
  m3plot2[m3plot1 >= cuts[i] & m3plot1 < cuts[i+1]] <- colfunc(10)[i]
}
i <- 10
m3plot2[m3plot1 >= cuts[i]] <- colfunc(10)[i]

m3plot3 <- rbind(Code, m3plot2)
lable4=row.names(m3plot3);lable4
...
pdf("Circos_Cell_type.pdf", width = 7, height = 7)
par(mar=c(0,0,0,0))
circos.clear();circos.par(canvas.xlim =c(-1.6,1.6),
                          canvas.ylim = c(-1.6,1.6),
                          cell.padding = c(0,0,0,0), 
                          gap.degree =60)
factors = "a"
circos.initialize(factors, xlim = c(0, ncol(m3plot3)))
...
text(0, 0.01, "Cell type\nenrichment", cex = 1)

draw.sector(0, 60, rou1 = 0.94, rou2=0.905,  col="white", border="white", lwd = 1, lty = 1)
draw.sector(359.5, 60.5, rou1 = 0.64, rou2=0.64,  border="grey50", lwd = 1, lty = 1)
draw.sector(359.5, 60.5, rou1 = 0.291, rou2=0.291,  border="grey50", lwd = 1, lty = 1, )
