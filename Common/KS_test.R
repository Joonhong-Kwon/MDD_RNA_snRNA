...
module_genes <- read.csv("kME0.7E_Module.csv")
...
write.csv(KStest, "Module_KS.csv", row.names = FALSE)
...
C <- read.csv ("each_KS.csv")

pdf("each_KS.pdf", width = 6.5, height = 4)
C$module <- factor(C$module, levels = c("Module1", "Module2", "Module3", "Module4", "Module5", "Module6"))
...
sp2<-ggplot(C, aes(x=Cluster, y=module, size=-log10(pvalue))) +
  ...
  scale_fill_manual(values = c("True" = "red", "False" = "gray"),
                    limits = c("True", "False")) +
  labs(title="each clusters (KS test)", x=NULL, y=NULL, fill="Significant", size = "-Log10 (FDR)")+
